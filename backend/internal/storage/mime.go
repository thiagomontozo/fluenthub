...
func DetectAllowed(header []byte,declared string)(string,error){detected:=http.DetectContentType(header);if _,ok:=allowedMIME[detected];!ok{return "",errors.New("file MIME is not allowed")};if declared!=""&&!strings.HasPrefix(declared,strings.Split(detected,";")[0]){return "",errors.New("declared MIME does not match content")};return detected,nil}
