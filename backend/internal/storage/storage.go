...
func (s *LocalObjectStorage) Open(_ context.Context, key string) (io.ReadCloser, error) {
	path, err := s.safePath(key)
	if err != nil {
		return nil, err
	}
...
func (s *LocalObjectStorage) Exists(_ context.Context, key string) (bool, error) {
	path, err := s.safePath(key)
	if err != nil {
		return false, err
	}
...
func (s *LocalObjectStorage) safePath(key string) (string, error) {
	...
	path := filepath.Clean(filepath.Join(s.root, filepath.FromSlash(key)))
